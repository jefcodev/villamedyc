...
while ($row_citas = $resultado_citas->fetch_assoc()) {
    ...
    while ($row_detalle = $resultado_detalle->fetch_assoc()) {
        ...
        $pdf->Cell(40, 6, utf8_decode($row_user['nombre'].' '.$row_user['apellidos'] ), 1, 0, 'C', 0);
        $pdf->Cell(55, 6, utf8_decode('FECHA:'), 1, 0, 'C', 1);
        $pdf->Cell(40, 6, utf8_decode($row_detalle['fecha']), 1, 0, 'C', 0);
        ...
        $pdf->Ln(); // Salto de línea
        $pdf->Cell(55, 6, utf8_decode('OBSERVACIONES'), 1, 0, 'C', 1);
        $pdf->MultiCell(135, 6, utf8_decode($row_detalle['observaciones']), 1, 'L', false);
        $cont++;
    }
}
